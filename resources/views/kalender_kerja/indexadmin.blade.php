<x-layout bodyClass="g-sidenav-show bg-gray-200">
    <x-navbars.sidebar activePage='kalender_kerjaadmin'></x-navbars.sidebar>
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        <x-navbars.navs.auth titlePage="Kalender Kerja - Admin"></x-navbars.navs.auth>
                ...
                <div class="col-xl-12 mb-4">
                    <div class="card">
                        <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                            ...
                            <!-- Filter Tanggal -->
                            <form method="GET" action="{{ route('admin.kalender_kerja.index') }}" class="d-flex">
                                ...
                                <div class="table-responsive p-4">
                                    <table class="table align-items-center table-hover table-bordered mb-0">
                                        <thead class="thead-light">
                                            <tr>
                                                <th>Nama Karyawan</th>
                                                @for ($i = 1; $i <= \Carbon\Carbon::parse($tanggalAkhir)->diffInDays($tanggalAwal) + 1; $i++)
                                                    <th>{{ \Carbon\Carbon::parse($tanggalAwal)->addDays($i - 1)->format('d M') }}
                                                    </th>
                                                @endfor
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($karyawan as $karyawanItem)
                                                <tr>
                                                    <td>{{ $karyawanItem->name }}</td>
                                                    @for ($i = 1; $i <= \Carbon\Carbon::parse($tanggalAkhir)->diffInDays($tanggalAwal) + 1; $i++)
                                                        @php
                                                            $tanggal = \Carbon\Carbon::parse($tanggalAwal)
                                                                ->addDays($i - 1)
                                                                ->toDateString();
                                                            $kalenderKaryawan = $kalenderKerja->get($karyawanItem->id);
                        ...
                        <div class="card-footer">
                            <!-- Form untuk Upload Kalender Kerja -->
                            ...
                            <form method="GET" action="{{ route('kalender_kerja.download_template.admin') }}">
                                <input type="hidden" name="tanggal_awal" value="{{ $tanggalAwal }}">
                                <input type="hidden" name="tanggal_akhir" value="{{ $tanggalAkhir }}">
