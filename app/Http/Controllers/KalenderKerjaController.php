...
class KalenderKerjaController extends Controller
{
    ...
    public function indexAdmin(Request $request)
    {
        ...
        $karyawan = \App\Models\User::all(); // Ambil semua karyawan tanpa memfilter berdasarkan unit atau divisi

        // Mengambil data kalender kerja berdasarkan filter tanggal
        $kalenderKerja = KalenderKerja::whereBetween('tanggal', [$tanggalAwal, $tanggalAkhir])
            ->get()
            ->groupBy('karyawan'); // Group by karyawan agar mudah ditampilkan nanti
        ...
        // Menampilkan view khusus admin dengan data yang sudah diambil
        return view('kalender_kerja.indexadmin', compact('kalenderKerja', 'tanggalAwal', 'tanggalAkhir', 'karyawan', 'shifts'));
    }
    ...
    public function uploadKalenderKerjakepalaunit(Request $request)
    {
        // Validasi file harus berupa Excel dan tanggal
        $request->validate([
            'import_file' => 'required|mimes:xlsx,xls',
            'tanggal_awal' => 'required|date',
            'tanggal_akhir' => 'required|date'
        ...
        // Cek apakah tanggal awal atau akhir berada di bulan yang sedang berjalan
        $currentMonth = Carbon::now()->month;
        if ($tanggalAwal->month == $currentMonth || $tanggalAkhir->month == $currentMonth) {
            return redirect()->back()->with('error', 'MAAF TIDAK BISA UPLOAD JADWAL PADA BULAN SAAT INI, SILAHKAN HUBUNGI MANAGEMENT');
        }
    ...
    public function downloadTemplateAdmin(Request $request)
    {
        // Mendapatkan filter tanggal dari request atau menggunakan tanggal bulan ini
        $tanggalAwal = $request->input('tanggal_awal', Carbon::now()->startOfMonth()->toDateString());
        $tanggalAkhir = $request->input('tanggal_akhir', Carbon::now()->endOfMonth()->toDateString());
        ...
        // Mengisi data karyawan
        $row = 2;  // Mulai dari baris kedua
        foreach ($karyawan as $index => $karyawanItem) {
            $sheet->setCellValue('A' . $row, $index + 1);
            $sheet->setCellValue('B' . $row, $karyawanItem->id);
            $sheet->setCellValue('C' . $row, $karyawanItem->name);
        ...
        // Simpan ke file Excel
        $fileName = 'template_kalender_kerja_admin.xlsx';
        $filePath = storage_path('app/public/' . $fileName);
